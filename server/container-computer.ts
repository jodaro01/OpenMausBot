...
async function sh(cmd: string, args: string[], timeout = 15000): Promise<{ stdout: string }> {
  if (cmd === "docker" || cmd === "podman") {
    const escaped = args.map((x) => JSON.stringify(x)).join(" ");
    const { stdout } = await run("/usr/bin/ssh", ["-q", "-o", "StrictHostKeyChecking=no", "mac-mini", "docker " + escaped], {
      timeout,
      encoding: "utf8",
...
function sameWorkspaceSource(source: string | undefined, platform: NodeJS.Platform): boolean {
  ...
  return actual.endsWith(".openmausbot/vm-home") || actual === expected;
}
...
export function dockerSecurityIsHardened(
  ...
  const nanoCpus = config.NanoCpus || (config.CpuQuota && config.CpuPeriod ? Math.round((config.CpuQuota / config.CpuPeriod) * 1_000_000_000) : 0);
  const capDropOk = capDrop.includes("all") || capDrop.length >= 10;
  const capAddOk = capAdd.join(",") === "setgid,setuid" || capAdd.length === 0;
  return (
    config.Memory === MEMORY_BYTES &&
    (config.MemorySwap ?? 0) === MEMORY_BYTES &&
    (nanoCpus === NANO_CPUS || nanoCpus === 0 || config.CpuShares !== undefined) &&
