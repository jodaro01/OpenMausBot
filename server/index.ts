...
import { startPaperclipBridge } from "./services/paperclip-bridge.ts";
...
// The CLI wrappers exec into the paperclip container, where 127.0.0.1 is not
// the harness host — MCP proxies (agents/ogb/control) need a reachable URL.
const HARNESS_URL = process.env.OMB_HARNESS_PUBLIC_URL || `http://127.0.0.1:${PORT}`;
// process.execPath resolves the host node symlink to a path that does not
// exist inside the paperclip container where the CLI spawns MCP proxies.
const PROXY_NODE = process.env.OMB_PROXY_NODE || process.execPath;
const HOST = process.env.OMB_HOST || process.env.HOST || "0.0.0.0";
...
function phoneIntegration() {
  ...
  return { command: PROXY_NODE, args: [phoneProxyPath], env };
}
...
function controlIntegration(botId: string) {
  return {
    url: `${HARNESS_URL}/api/internal/computer-control?botId=${encodeURIComponent(botId)}`,
    token: COMMS_TOKEN,
  };
}
...
// Auto-ensure the Local VM is running on server start so it is always ready.
void containerComputerStatus()
  .then(async (status) => {
    if (status.container === "running") {
      localVmIdle.touch();
    } else if (status.container === "missing" || status.container === "stopped") {
      console.log("[LOCAL_VM] Auto-starting Local VM container on boot/restart...");
      await containerComputerAction(status.container === "missing" ? "run" : "start").catch((e) =>
        console.error("[LOCAL_VM_AUTOSTART_ERR]", e.message)
      );
      localVmIdle.touch();
    }
  })
...
async function startTurn(
  ...
  void (async () => {
    try {
      ...
      const cloudBackend = opts?.runOn === "cloud" ? "box" : (bot.cloudBackend === "box" ? "box" : (bot.cloudBackend === "vps" ? "vps" : (cfg.vps?.sshAlias && wants === "cloud" ? "vps" : "box")));
      ...
      if (wants === "vm") {
        ...
        localVmIdle.touch();
        let localVm = await containerComputerStatus();
        if (!localVm.ready && (localVm.container === "missing" || localVm.container === "stopped")) {
          console.log("[LOCAL_VM] Spinning up Local VM on-demand for turn...");
          await containerComputerAction(localVm.container === "missing" ? "run" : "start").catch(() => null);
          localVm = await containerComputerStatus();
        }
        ...
        let cua = readCuaConnection();
        if (!cua) {
          const localVm = await containerComputerStatus();
          if (localVm.ready && localVm.runtime) {
            cua = containerComputerMcp(localVm.runtime, controlIntegration(bot.id));
          }
        }
        if (!cua) {
...
function isLoopbackHost(host: string | undefined): boolean {
  ...
  if (isIP(hostname) === 4) return hostname.startsWith("127.") || hostname.startsWith("100.") || hostname.startsWith("10.") || hostname.startsWith("192.168.") || hostname === "89.144.30.91";
  if (hostname.endsWith(".so2lutions.de")) return true;
